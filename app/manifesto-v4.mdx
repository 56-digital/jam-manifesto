...
The deliberate arrangement and re-arrangement of cables from one point to another creates music. The slowness is not an artifact of the process but the process itself.
...
The building blocks are the same ones that have existed since the first synthesizer was wired together in a lab: oscillators, filters, envelopes, buses, gates, control voltage. The same primitives that Stockhausen worked with at WDR, the same signal chain you would patch together by hand on a modular synth.
